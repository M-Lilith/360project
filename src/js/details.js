...
(function () {
    ...
    Magnifier.prototype = {
        ...
        createRelativeBox: function () {
            ...
            this.asideBox.style.cssText = 'position:absolute;left:105%;top:50%;border:2px solid #ccc;transform:translateY(-50%);background-color: white;' +
        ...
        calcPosition: function (e) {
            var left = e.pageX - this.ele.offsetLeft - this.maskLayerWidth,
                top = e.pageY - this.ele.offsetTop - this.maskLayerHeight;
            if (left < 0) {
                left = 0;
...
// 360生活馆
$.get('../json/poster.json', (data) => {
    $(".poster").html(
        `
        <a href=${data.src}><img src=${data.poster1} alt=""></a>
        <a href=${data.src}><img src=${data.poster2} alt=""></a>
        `
    )
})
...
for (var i = 0; i < aList.length; i++) {
    ...
    aList[i].onmouseover = function () {
        for (var j = 0; j < divList.length; j++) {
            ...
            aList[j].style.backgroundColor = "pink";
        }
...
// tab切换图片内容
$.get('../json/routerlist.json', (data) => {
    $("#content  #tab-img").html(`
    <img src=${data.routerlist01}><img src=${data.routerlist02}><img src=${data.routerlist03}><img src=${data.routerlist04}>
    <img src=${data.routerlist05}><img src=${data.routerlist06}><img src=${data.routerlist07}><img src=${data.routerlist08}>
    `)
})
// 
$.get('../json/routerlist.json', (data) => {
    $("#content #tab-spe").html(`<img src=${data.specification}>`)
})
// 
$.get('../json/routerlist.json', (data) => {
    $("#content #tab-spd").html(`<img src=${data.routerlist08}>`)
})
// 吸顶
$(window).scroll(function () {
    if ($(window).scrollTop() >= 1446) {
        $("#menu").css({
            'position': 'sticky',
            'top': '40px',
        });
